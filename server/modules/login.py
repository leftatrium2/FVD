...
def login():
    ...
    count = db.session.query(IvdUserList) \
        .filter(and_(IvdUserList.user_name == user_name, IvdUserList.password == md5(password))) \
        .count()
    if count <= 0:
        return result_failure(const.LOGIN_ERR_USER_OR_PASSWORD, "LOGIN_ERR_USER_OR_PASSWORD")
    res = db.session.query(IvdOnlineList.user_name,
                           IvdOnlineList.valid_time,
                           IvdOnlineList.token,
                           IvdUserList.avatar,
                           IvdUserList.nickname,
                           IvdUserList.roles,
                           IvdUserList.permissions) \
        .select_from(IvdOnlineList) \
        .join(IvdUserList, IvdUserList.user_name == IvdOnlineList.user_name) \
