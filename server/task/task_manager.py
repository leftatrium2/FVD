...
from model.model import IvdTaskList, IvdTaskQueue
...
# 将当前的单次或者批量任务，放到ivd_task_queue表中
class TaskManager(object):
    ...
    def get_task(self) -> dict or None:
        with get_db_client() as db:
            with db.begin():
                res = db.query(IvdTaskQueue) \
                    .filter(IvdTaskQueue.is_deleted == 0, IvdTaskQueue.status == task_const.TASK_STATUS_QUEUE) \
                    .order_by(IvdTaskQueue.create_time.desc()) \
                    .with_for_update() \
                    .first()
                if res:
                    db.query(IvdTaskQueue) \
                        .filter(IvdTaskQueue.id == res.id) \
                        .update({IvdTaskQueue.status: task_const.TASK_STATUS_RUNNING})
    ...
    def add_batch_task(self, task: IvdTaskList):
        if not task:
    ...
    # 添加一个task
    # 简单任务，立即执行类型，单独的URL
    def add_simple_task(self, task: IvdTaskList):
        if not task:
            return False
    ...
    def _query_scheduled_task(self) -> list:
        with get_db_client() as db:
            res_list = db.query(IvdTaskList) \
                .filter(
                and_(IvdTaskList.is_deleted == 0,
                     IvdTaskList.task_execution_mode == task_const.TASK_EXECUTION_BATCH_MODE)) \
                .all()
            return res_list
